...
The [VMs at Hetzner](https://www.hetzner.com/de/cloud) were in a totally different league. I chose a server with the following specs:
```
vServer:        CX31
...
We'll use ssh to access the server. [Here's a good blog entry on how to setup a connection.](https://www.digitalocean.com/community/tutorials/how-to-configure-ssh-key-based-authentication-on-a-linux-server)
...
We need Java for Paper, Git and build-essential for mcrcon (more on that later). The current version of [Paper supports Java 17](https://docs.papermc.io/paper/getting-started):
...
{{<icon "triangle-exclamation">}} Disclaimer, before installing paper: I'm using `paper-1.20.1-33`. Change it to your preferred version. [Get the newest version](https://papermc.io/downloads/paper) or use the [build explorer](https://papermc.io/downloads/all) to find a specific minecraft version.
...
Now let's initialize the server. You'll need a text editor for the next step. I prefer nano for its simplicity ([Short nano Tutorial](https://linuxize.com/post/how-to-use-nano-text-editor/)), but there are lots of alternative options.
...
We'll use [Aikars Flags](https://docs.papermc.io/paper/aikars-flags) to optimize the Java process. Use the following configuration as a template:
...
Checkout [SpigotMC](https://www.spigotmc.org/) or [Carftaro](https://craftaro.com/) for cool plug-ins. A basic set may be the follwing:

- [UltimateTimber](https://craftaro.com/marketplace/product/ultimatetimber.18) a beautiful plugin to cut down trees whole easier.
- [Harbor](https://www.spigotmc.org/resources/harbor-a-sleep-enhancement-plugin.60088/) allows to skip the night if enough players go to bed.
- [CoreProtect](https://www.spigotmc.org/resources/coreprotect.8631/) logs every block change and traces who is responsible for griefing. Enables Administrators to revert damages.
- [GSit](https://www.spigotmc.org/resources/gsit-modern-sit-seat-and-chair-lay-and-crawl-plugin-1-13-1-20-2.62325/) is a fun plugin to sit on chairs or lay on the floor.
...
Download the newest Paper Version ([Get the newest version](https://papermc.io/downloads/paper)):
```
wget https://api.papermc.io/v2/projects/paper/versions/1.20.1/builds/33/downloads/paper-1.20.1-33.jar
```
...
Use [Minecraft commands](https://minecraft.fandom.com/wiki/Commands#List_and_summary_of_commands), but without the slash `/`.
